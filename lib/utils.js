 ...
 */
utils.setSuppressWarnings = function(env) {
	if (env.SUPPRESS_NO_CONFIG_WARNING) {
		showWarnings = false;
	}
};

/**
 * Log Warnings (or suppress)
 * @param {String} message - warnings message
 */
...
utils.strictModeCheck = function (env, foundFiles) {
	var check = true,
		last,
		regEnv  = new RegExp('(?:^|-)' + env.NODE_ENV),
		regIns  = new RegExp('-' + env.NODE_APP_INSTANCE),
		regHost = new RegExp('(?:^|-)' + env.HOSTNAME);

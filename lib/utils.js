...
/**
* @private
* extract calling module filename from 2nd line in err.stack
*/
utils._filenameFromError = function (err, n) {
  var filename = (err.stack || '').split(/\n/)[n || 1]
  if (filename) {
    return filename.replace(/^.*?\(((?:[A-Za-z]:\\|\/)[^:]+?):.*$/, '$1')
  }
}
